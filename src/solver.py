...
from utils import create_instance_from_parents, create_random_genes, get_parents, sort_by_score, check_diagonal
...
class GeneticAlgorithmSolver:
    ...
    def fitness(self, instance: GAInstance) -> int:
        ...
        for col in range(CHESSBOARD_LENGHT):
            ...
            #
            # checking diagonals we have two possibilties:
            ...
            #   2. queen is in the edges and has two diagonals
                ...
                if (col == CHESSBOARD_FISRT_POSITION and row == CHESSBOARD_FISRT_POSITION) or (row == CHESSBOARD_FISRT_POSITION):
                    score -= check_diagonal(row, col, CHESSBOARD_DOWN_DIAGONAL)

                elif (col == CHESSBOARD_FISRT_POSITION and row == CHESSBOARD_LAST_POSITION) or (row == CHESSBOARD_LAST_POSITION):
                    score -= check_diagonal(row, col, CHESSBOARD_UP_DIAGONAL)
        ...
        for child in children:
            for gene in child:
                if random() < self._mutation_probability:
                    child[child.index(gene)] = randint(CHESSBOARD_FISRT_POSITION, CHESSBOARD_LAST_POSITION)
    ...
    def fit(self):
        ...
        for i in range(self._max_iterations):
            print(f'{top_one.gen}° gen, id: {top_one.id}, gene: {top_one.gene}')
            self._generations_counter += 1
